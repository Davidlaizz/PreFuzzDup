...
class Server {
 public:
  ...
  void setup(std::size_t capacity, double fpp, std::map<std::string,int> thresholds) {
    ...
    if (capacity == 0) fail("Prefix Filter capacity must be positive");
    schema();
    std::ostringstream os; bool first = true; for (const auto& [t,d] : thresholds) { if (!first) os << ','; os << t << '=' << d; first = false; }
    Stmt count(db_,"SELECT count(*) FROM records");count.step_row();const auto existing = count.col_int(0);
    if (existing > 0) {
      Stmt old(db_,"SELECT v FROM meta WHERE k='thresholds'");
      if (!old.step_row() || old.col_text(0) != os.str()) fail("cannot change search thresholds after records are indexed; rebuild the index first");
      if (static_cast<std::uint64_t>(existing) > capacity) fail("Prefix Filter capacity is smaller than the existing record count");
    }
    db_.exec("BEGIN IMMEDIATE"); try { put_meta("capacity", std::to_string(capacity)); put_meta("fpp", std::to_string(fpp)); put_meta("thresholds", os.str()); db_.exec("COMMIT"); } catch (...) { try { db_.exec("ROLLBACK"); } catch (...) {} throw; }
    load_config(); rebuild_filters();
  }
  ...
  void insert_record(const Record& r) {
    ...
    if (record_counts_[r.type] >= capacity_) {
      if (capacity_ > std::numeric_limits<std::size_t>::max() / 2) fail("Prefix Filter capacity cannot be expanded further");
      capacity_ *= 2;
      db_.exec("BEGIN IMMEDIATE"); try { put_meta("capacity", std::to_string(capacity_)); db_.exec("COMMIT"); } catch (...) { try { db_.exec("ROLLBACK"); } catch (...) {} throw; }
      rebuild_filters();
    }
    ...
    db_.exec("BEGIN IMMEDIATE"); try {
      ...
      db_.exec("COMMIT"); ++record_counts_[r.type];
 ...
 private:
  Db db_; fs::path blobs_; std::size_t capacity_{}; double fpp_{}; std::map<std::string,int> thresholds_; std::unordered_map<std::string,PrefixFilter> filters_; std::unordered_map<std::string,std::size_t> record_counts_;
  ...
  void rebuild_filters(){filters_.clear();record_counts_.clear();for(const auto&[t,d]:thresholds_)for(int j=0;j<=d;++j)filters_.emplace(filter_key(t,j),PrefixFilter(capacity_,fpp_));Stmt q(db_,"SELECT type,tag FROM records");while(q.step_row()){auto t=q.col_text(0);auto tag=q.col_blob(1);auto it=thresholds_.find(t);if(it==thresholds_.end())fail("record type is missing from search configuration");++record_counts_[t];for(int j=0;j<=it->second;++j)filters_[filter_key(t,j)].insert(segment(tag,j,it->second+1));}for(const auto&[t,n]:record_counts_)if(n>capacity_)fail("Prefix Filter capacity is smaller than indexed record count for type: "+t);}
};
...
void self_test(){
  FuzzyExtractor fe;
  PrefixFilter filter(100,0.01);
  for(std::uint32_t i=0;i<1000;++i){Bytes value{static_cast<std::uint8_t>(i),static_cast<std::uint8_t>(i>>8),static_cast<std::uint8_t>(i>>16),static_cast<std::uint8_t>(i>>24)};filter.insert(value);}
  for(std::uint32_t i=0;i<1000;++i){Bytes value{static_cast<std::uint8_t>(i),static_cast<std::uint8_t>(i>>8),static_cast<std::uint8_t>(i>>16),static_cast<std::uint8_t>(i>>24)};if(!filter.maybe_contains(value))fail("Prefix Filter false negative after bin overflow");}
  ...
  try { Server s(root/"state.sqlite",root/"blobs");s.setup(100,0.01,{{"text",6}});const Bytes f=bytes("A short message about encrypted cloud storage and fuzzy deduplication.");const Bytes tag=make_tag(f,"text");auto r=first_upload(s,"alice","text",f,tag,fe);auto found=s.candidates("text",tag);if(found.size()!=1||found[0].r.rid!=r.rid)fail("Prefix Filter and disk posting lookup failed");auto q=fuzzy_pow(s,"bob",r,f,tag,fe);auto restored=q?recover(s,r,*q,tag,fe):std::nullopt;if(!restored||!equal_ct(*restored,f))fail("end-to-end protocol self-test failed");s.bind("bob",r.rid);if(s.refs("bob").size()!=1)fail("reference-binding self-test failed");std::error_code ec;fs::remove_all(root,ec); }
